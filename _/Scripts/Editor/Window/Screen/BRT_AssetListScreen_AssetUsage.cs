...
namespace BuildReportRemastered.Window.Screen
{
    public partial class AssetList
    {
        ...
        string GetPrettyAssetPath(string assetPath, bool showAssetPath, bool selected)
        {
            ...
            {
                var path = BuildReportRemastered.Util.GetAssetPath(assetPath);
                var filename = BuildReportRemastered.Util.GetAssetFilename(assetPath);

                entry.AssetPathSelected = string.Format("<color=#{0}>{1}</color><color=white><b>{2}</b></color>",
                    BuildReportRemastered.Window.Screen.AssetList.GetPathColor(true),
                    path, filename);
                ...
                if (selected)
                {
                    if (string.IsNullOrEmpty(entry.AssetPathSelected) || entry.NumberForPathSelected != number)
                    {
                        entry.AssetPathSelected = string.Format(" {0}. <color=#{1}>{2}</color><b>{3}</b>",
                            (number + 1).ToString(),
                            BuildReportRemastered.Window.Screen.AssetList.GetPathColor(true),
                            BuildReportRemastered.Util.GetAssetPath(assetPath),
                            BuildReportRemastered.Util.GetAssetFilename(assetPath));
            ...
            else
            {
                if (string.IsNullOrEmpty(entry.AssetName) || entry.NumberForName != number)
                {
                    entry.AssetName = string.Format(" {0}. <b>{1}</b>",
                        (number + 1).ToString(), BuildReportRemastered.Util.GetAssetFilename(assetPath));
        ...
        public void DrawAssetUsage(Rect position, BuildReportRemastered.AssetList listToDisplay,
                    ...
                    case ASSET_USAGE_DISPLAY_END:
                        {
                            ...
                            {
                                BuildReportRemastered.AssetDependencies.PopulateAssetEndUsers(selectedAsset.Name, assetDependencies);
                            }
                    ...
                    case ASSET_USAGE_DISPLAY_END:
                        {
                            ...
                            if (endUsersList.Count == 0)
                            {
                                BuildReportRemastered.AssetDependencies.PopulateAssetEndUsers(selectedAsset.Name,
            ...
            out int newEntryHoveredIdx, out string newEntryHoveredAssetPath, out Rect newEntryHoveredRect)
        {
            ...
            for (; directUserN < directUserLen; ++directUserN)
            {
                ...
                _assetUsageEntryLabel.text =
                    GetPrettyAssetPath(assetPath, BuildReportRemastered.Options.ShowColumnAssetPath, _selectedAssetUserIdx == directUserN);
            ...
            out int newEntryHoveredIdx, out string newEntryHoveredAssetPath, out Rect newEntryHoveredRect)
        {
            ...
            for (; endUserN < endUserLen; ++endUserN)
            {
                ...
                _assetUsageEntryLabel.text =
                    GetPrettyAssetPath(assetPath, BuildReportRemastered.Options.ShowColumnAssetPath, _selectedAssetUserIdx == endUserN);
            ...
            out int newEntryHoveredIdx, out string newEntryHoveredAssetPath, out Rect newEntryHoveredRect)
        {
            ...
            for (; userFlattenedN < userFlattenedLen; ++userFlattenedN)
            {
                ...
                if (usersFlattened[userFlattenedN].IndentLevel > 1)
                {
                    ...
                    int idxOfPrevious;
                    if (BuildReportRemastered.AssetDependencyGenerator.IsFileTypeBeforeAnother(usersFlattened, userFlattenedN,
                ...
                _assetUsageEntryLabel.text = GetPrettyAssetPath(usersFlattened[userFlattenedN].AssetPath, BuildReportRemastered.Options.ShowColumnAssetPath,
            ...
            AssetDependencies assetDependencies)
        {
            ...
            else if (_selectedAssetUsageDisplayIdx == ASSET_USAGE_DISPLAY_DIRECT)
            {
                ...
                _assetUsageEntryLabel.text = BuildReportRemastered.Util.GetAssetFilename(_assetUserCrumbs[0].AssetPath);
                ...
                {
                    _assetUsageEntryLabel.text = BuildReportRemastered.Util.GetAssetFilename(_assetUserCrumbs[crumbN].AssetPath);
            ...
            AssetDependencies assetDependencies)
        {
            ...
            for (int n = 0, len = _assetUsageAncestry.Count; n < len; ++n)
            {
                ...
                if (BuildReportRemastered.Options.IsAssetUsageLabelTypeOnVerbose)
                {
                    if (n == 0)
                    {
                ...
                else if (BuildReportRemastered.Options.IsAssetUsageLabelTypeOnStandard)
                {
                    if (n < len - 1)
                    {
                ...
                else if (BuildReportRemastered.Options.IsAssetUsageLabelTypeOnMinimal)
                {
                    if (n < len - 1)
                    {
                        widthToAdd = assetUsageArrow.width + assetUsageArrowStyle.padding.horizontal;
