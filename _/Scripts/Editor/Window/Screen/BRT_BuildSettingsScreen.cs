...
namespace BuildReportRemastered.Window.Screen
{
    public class BuildSettings : BaseScreen
    {
        ...
        void DrawSetting(string name, string val, bool showEvenIfEmpty = true)
        {
            ...
            var nameStyle = GUI.skin.FindStyle(BuildReportRemastered.Window.Settings.SETTING_NAME_STYLE_NAME);
            if (nameStyle == null)
            {
                nameStyle = GUI.skin.label;
            }

            var valueStyle = GUI.skin.FindStyle(BuildReportRemastered.Window.Settings.SETTING_VALUE_NO_WRAP_STYLE_NAME);
            if (valueStyle == null)
            {
        ...
        {
            var titleStyle = GUI.skin.FindStyle(BuildReportRemastered.Window.Settings.INFO_TITLE_STYLE_NAME);
            if (titleStyle == null)
            {
        ...
        void DrawPackageSettings(BuildInfo buildReportToDisplay, UnityBuildSettings settings)
        {
            ...
            var valueStyle = GUI.skin.FindStyle(BuildReportRemastered.Window.Settings.SETTING_VALUE_STYLE_NAME);
            if (valueStyle == null)
            {
                valueStyle = GUI.skin.label;
            }
            ...
            if (!packageListIsEmpty)
            {
                ...
                for (int n = 0, len = packageList.Count; n < len; ++n)
                {
                    ...
                    if (!string.IsNullOrEmpty(packageList[n].Location) && packageList[n].Location != BuildReportRemastered.UnityBuildSettingsUtility.DEFAULT_REGISTRY_URL)
                    {
        ...
        void DrawPackagePingButton(BuildReportRemastered.UnityBuildSettings.PackageEntry packageEntry)
        {
            if (!string.IsNullOrEmpty(packageEntry.LocalPath))
            {
                ...
                {
                    BuildReportRemastered.Util.OpenInFileBrowser(packageEntry.LocalPath);
                }
            }
        }
        ...
        )
        {
            ...
            var fileFilterPopupStyle = GUI.skin.FindStyle(BuildReportRemastered.Window.Settings.FILE_FILTER_POPUP_STYLE_NAME);
            ...
            BuildReportRemastered.Options.ShowProjectSettingsInMultipleColumns = GUILayout.Toggle(BuildReportRemastered.Options.ShowProjectSettingsInMultipleColumns, "Multiple Columns");
            GUILayout.Space(30);
            ...
            var showMultiColumn = BuildReportRemastered.Options.ShowProjectSettingsInMultipleColumns;
