...
namespace BuildReportRemastered
{
    public partial class BuildInfo
    {
        ...
        public string GetTimeReadable()
        {
            ...
            return TimeGot.ToString(BuildReportRemastered.ReportGenerator.TIME_OF_BUILD_FORMAT);
        }
        ...
        {
            return BuildReportRemastered.Util.GetBuildInfoDefaultFilename(ProjectName, BuildType, BuildTimeGot);
        }

        public string GetAccompanyingAssetDependenciesFilename()
        {
            return BuildReportRemastered.Util.GetAssetDependenciesDefaultFilename(ProjectName, BuildType, BuildTimeGot);
        }
        ...
        public void RecategorizeAssetLists()
        {
            ...
            {
                fileFiltersToUse = BuildReportRemastered.FiltersUsed.GetProperFileFilterGroupToUse();
            ...
            if (UsedAssets != null)
            {
                UsedAssets.AssignPerCategoryList(
                    BuildReportRemastered.ReportGenerator.SegregateAssetSizesPerCategory(UsedAssets.All, fileFiltersToUse));

                UsedAssets.RefreshFilterLabels(fileFiltersToUse);

                UsedAssets.ResortDefault(BuildReportRemastered.Options.NumberOfTopLargestUsedAssetsToShow);
            }

            if (UnusedAssets != null)
            {
                UnusedAssets.AssignPerCategoryList(
                    BuildReportRemastered.ReportGenerator.SegregateAssetSizesPerCategory(UnusedAssets.All, fileFiltersToUse));

                UnusedAssets.RefreshFilterLabels(fileFiltersToUse);

                UnusedAssets.ResortDefault(BuildReportRemastered.Options.NumberOfTopLargestUnusedAssetsToShow);
            }
        }
        ...
        void CalculateUsedAssetsDerivedSizes()
        {
            if (UsedAssets != null)
            {
                ...
                {
                    UsedAssets.All[n].DerivedSize = BuildReportRemastered.Util.GetApproxSizeFromString(UsedAssets.All[n].Size);
                }
            }
        }
        ...
        {
            System.Array.Sort(BuildSizes, delegate (BuildReportRemastered.SizePart b1, BuildReportRemastered.SizePart b2)
            {
        ...
        long GetSizeSumForUsedAssets(string assetFolderName, System.Func<string, bool> fileTypePredicate)
        {
            ...
            return UsedAssets.All.Where(part =>
                                 BuildReportRemastered.Util.IsFileInAPath(part.Name, assetFolderName) &&
                                 fileTypePredicate(part.Name))
                             .Sum(part => BRT_LibCacheUtil.GetImportedFileSize(part.Name));
        }

        static void AddToSize(BuildReportRemastered.SizePart buildSize, long sizeToAdd)
        {
            if (buildSize != null)
            {
                buildSize.DerivedSize += sizeToAdd;
                buildSize.Size = BuildReportRemastered.Util.GetBytesReadable(buildSize.DerivedSize);
            }
        }
        ...
        void AddToSize(string buildSizeName, long sizeToAdd)
        {
            ...
            BuildReportRemastered.SizePart buildSize = BuildSizes.FirstOrDefault(part => part.Name == buildSizeName);
        ...
        void AddToTotalSize(long sizeToAdd)
        {
            ...
            BuildReportRemastered.SizePart buildSize = BuildSizes.FirstOrDefault(part => part.IsTotal);
        ...
        void ChangeTotalSize(double newSize)
        {
            ...
            BuildReportRemastered.SizePart totalSize = BuildSizes.FirstOrDefault(part => part.IsTotal);
            ...
            if (totalSize != null)
            {
                ...
                totalSize.Size = BuildReportRemastered.Util.GetBytesReadable(totalSize.DerivedSize);
