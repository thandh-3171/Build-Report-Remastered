...
public static class BRT_LibCacheUtil
{
    ...
    public static long GetImportedFileSize(string assetPath)
    {
        ...
        assetPath = BuildReportRemastered.Util.MyHtmlDecode(assetPath);

        // files in "StreamingAssets" folder do not get imported
        // in the 1st place, so skip them
        if (BuildReportRemastered.Util.IsFileStreamingAsset(assetPath))
        {
            return -1;
        }

        // files like Thumbs.db or .DS_Store files do not get imported
        if (BuildReportRemastered.Util.IsUselessFile(assetPath))
        {
            return -1;
        }

        // Unix-style hidden files do not get imported
        if (BuildReportRemastered.Util.IsFileAUnixHiddenFile(assetPath))
        {
            return -1;
        }
        ...
        if (!string.IsNullOrEmpty(assetPath))
        {
            ...
            {
                result = BuildReportRemastered.Util.GetFileSizeInBytes(assetImportedPath);
            }
            else
            {
